    ...
    <main class="pt-6">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col lg:flex-row gap-10 mb-12">
            ...
            <div class="w-full lg:w-2/3 space-y-8">
                ...
                {{-- ══ ULASAN PENGUNJUNG ══ --}}
                <div class="border-b border-gray-100 pb-12 pt-8" id="reviews-section">
                    <div class="flex items-center gap-3 mb-8">
                        <div class="w-12 h-12 rounded-2xl bg-gradient-to-br from-yellow-400 to-orange-500 flex items-center justify-center text-white shadow-lg shadow-orange-500/20">
                            <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>
                        </div>
                        <h2 class="text-2xl md:text-3xl font-black text-gray-900 tracking-tight">Ulasan Pengunjung</h2>
                    </div>

                    @if(session('success'))
                        <div class="mb-8 bg-emerald-50 text-emerald-700 p-5 rounded-2xl border border-emerald-200 font-medium text-sm flex items-center gap-3 shadow-sm">
                            <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                            {{ session('success') }}
                        </div>
                    @endif
                    @if(session('error'))
                        <div class="mb-8 bg-red-50 text-red-700 p-5 rounded-2xl border border-red-200 font-medium text-sm flex items-center gap-3 shadow-sm">
                    ...
                    @auth
                        ...
                        @endphp
                        @if(!$userHasReviewed)
                            <div class="bg-white rounded-3xl p-6 md:p-8 border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] mb-10 relative overflow-hidden">
                                <div class="absolute top-0 right-0 w-32 h-32 bg-gradient-to-bl from-[#1a6bbf]/5 to-transparent rounded-bl-full pointer-events-none"></div>
                                ...
                                <form action="{{ route('review.store', $place->id) }}" method="POST" enctype="multipart/form-data">
                                    @csrf
                                    <div class="mb-6">
                                        <label class="block text-sm font-bold text-gray-700 mb-3">Berapa bintang untuk destinasi ini?</label>
                                        <div class="flex items-center gap-2" id="star-rating-container">
                                            @for($i=1; $i<=5; $i++)
                                                <button type="button" onclick="setRating({{ $i }})" class="star-btn focus:outline-none transform transition-transform duration-200 hover:scale-125" data-rating="{{ $i }}">
                                                    <svg class="w-10 h-10 text-gray-200 hover:text-yellow-400 transition-colors duration-200 fill-current drop-shadow-sm" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>
                                                </button>
                                            @endfor
                                        </div>
                                    ...
                                    <div class="mb-6">
                                        <label class="block text-sm font-bold text-gray-700 mb-3">Ceritakan detailnya</label>
                                        <textarea name="content" rows="4" class="w-full rounded-2xl border-gray-200 bg-gray-50 focus:bg-white shadow-inner focus:border-[#1a6bbf] focus:ring focus:ring-[#1a6bbf]/20 text-gray-700 p-4 transition-all resize-none" placeholder="Apa yang paling Anda sukai dari tempat ini? Berikan tips untuk pengunjung lain..." required></textarea>
                                    </div>
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                                        <div>
                                            <label class="block text-sm font-bold text-gray-700 mb-3">Tipe Kunjungan (Opsional)</label>
                                            <select name="visit_type" class="w-full rounded-xl border-gray-200 bg-gray-50 focus:bg-white shadow-sm focus:border-[#1a6bbf] focus:ring focus:ring-[#1a6bbf]/20 text-gray-700 p-3.5 transition-all appearance-none bg-[url('data:image/svg+xml;charset=US-ASCII,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20width%3D%22292.4%22%20height%3D%22292.4%22%3E%3Cpath%20fill%3D%22%23131313%22%20d%3D%22M287%2069.4a17.6%2017.6%200%200%200-13-5.4H18.4c-5%200-9.3%201.8-12.9%205.4A17.6%2017.6%200%200%200%200%2082.2c0%205%201.8%209.3%205.4%2012.9l128%20127.9c3.6%203.6%207.8%205.4%2012.8%205.4s9.2-1.8%2012.8-5.4L287%2095c3.5-3.5%205.4-7.8%205.4-12.8%200-5-1.9-9.2-5.5-12.8z%22%2F%3E%3C%2Fsvg%3E')] bg-[length:12px_12px] bg-no-repeat bg-[position:right_1rem_center]">
                                                <option value="">Pilih dengan siapa Anda pergi...</option>
                                        ...
                                        <div>
                                            <label class="block text-sm font-bold text-gray-700 mb-3">Foto Kenangan (Opsional)</label>
                                            <div class="relative w-full">
                                                <input type="file" name="image" id="review-image" accept="image/*" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10" onchange="document.getElementById('file-name').textContent = this.files[0] ? this.files[0].name : 'Pilih foto...'"/>
                                                <div class="w-full rounded-xl border border-dashed border-gray-300 bg-gray-50 flex items-center gap-3 p-3.5 text-gray-500 transition-colors group-hover:border-[#1a6bbf] relative overflow-hidden">
                                                    <div class="w-8 h-8 rounded-full bg-[#1a6bbf]/10 flex items-center justify-center text-[#1a6bbf] flex-shrink-0">
                                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                                                    </div>
                                                    <span id="file-name" class="text-sm truncate">Pilih foto (Maks. 2MB)</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="flex justify-end pt-4 border-t border-gray-100">
                                        <button type="submit" class="bg-gradient-to-r from-[#1a6bbf] to-[#145299] hover:from-[#145299] hover:to-[#0f407a] text-white font-bold px-10 py-4 rounded-full transition-all shadow-[0_8px_20px_rgba(26,107,191,0.25)] hover:shadow-[0_12px_25px_rgba(26,107,191,0.35)] transform hover:-translate-y-1">
                        ...
                        @else
                            <div class="bg-gradient-to-r from-blue-50 to-indigo-50 p-6 rounded-2xl border border-blue-100 mb-10 flex items-center gap-4 shadow-sm">
                                ...
                                <div>
                                    <h4 class="font-bold text-gray-900">Terima kasih atas ulasan Anda!</h4>
                                    <p class="text-sm text-gray-600 mt-1">Ulasan Anda membantu jutaan traveler lainnya dalam merencanakan liburan mereka di Sukabumi.</p>
                                </div>
                            </div>
                        @endif
                    @else
                        <div class="bg-gradient-to-br from-gray-50 to-white rounded-3xl p-8 border border-gray-200 mb-10 text-center shadow-sm relative overflow-hidden">
                            ...
                            <div class="relative z-10">
                                <div class="w-16 h-16 mx-auto bg-blue-50 rounded-full flex items-center justify-center text-[#1a6bbf] mb-4">
                                ...
                                <h3 class="text-xl font-bold text-gray-900 mb-2">Ingin membagikan pengalaman Anda?</h3>
                                <p class="text-gray-500 mb-6 text-sm max-w-md mx-auto">Masuk ke akun Anda untuk menulis ulasan dan membantu traveler lain menemukan tempat terbaik di Sukabumi.</p>
                                <a href="{{ route('login') }}" class="inline-flex items-center justify-center bg-[#1a6bbf] hover:bg-[#145299] text-white font-bold px-8 py-3.5 rounded-full transition-all shadow-md hover:shadow-lg transform hover:-translate-y-0.5">
                    ...
                    @if($place->reviews->count() > 0)
                        <div class="space-y-6">
                            @foreach($place->reviews as $review)
                                <div class="bg-white border border-gray-100 rounded-3xl p-6 md:p-8 shadow-[0_4px_20px_rgb(0,0,0,0.03)] relative transition hover:shadow-[0_8px_30px_rgb(0,0,0,0.06)]">
                                    <svg class="absolute top-6 right-6 w-12 h-12 text-gray-50" fill="currentColor" viewBox="0 0 24 24"><path d="M14.017 21v-7.391c0-5.704 3.731-9.57 8.983-10.609l.995 2.151c-2.432.917-3.995 3.638-3.995 5.849h4v10h-9.983zm-14.017 0v-7.391c0-5.704 3.748-9.57 9-10.609l.996 2.151c-2.433.917-3.996 3.638-3.996 5.849h3.983v10h-9.983z"/></svg>
                                    
                                    <div class="relative z-10">
                                        <div class="flex items-center gap-4 mb-5">
                                            <div class="w-14 h-14 rounded-full bg-gradient-to-br from-[#1a6bbf] to-[#0f407a] text-white flex items-center justify-center font-bold text-xl shadow-inner border-2 border-white ring-2 ring-gray-100">
                                                {{ strtoupper(substr($review->user->name ?? 'A', 0, 1)) }}
                                            </div>
                                            <div>
                                                <p class="font-bold text-gray-900 text-lg">{{ $review->user->name ?? 'Anonim' }}</p>
                                                <div class="flex items-center text-sm text-gray-500 gap-2 mt-0.5">
                                                    ...
                                                    @if($review->visit_type)
                                                        ...
                                                        <span class="inline-flex items-center text-xs font-semibold px-2 py-0.5 rounded-md bg-gray-100 text-gray-600">
                                                            {{ $review->visit_type }}
                                                        </span>
                                                    @endif
                                                </div>
                                        ...
                                        <div class="flex text-yellow-400 mb-4 drop-shadow-sm">
                                            @for($i=1; $i<=5; $i++)
                                        ...
                                        <p class="text-base text-gray-700 leading-relaxed italic">"{{ $review->content }}"</p>
                                        
                                        @if($review->image_path)
                                            <div class="mt-5 overflow-hidden rounded-2xl border border-gray-100">
                                                <img src="{{ Storage::url($review->image_path) }}" alt="Foto ulasan dari {{ $review->user->name ?? 'Anonim' }}" class="w-full max-h-72 object-cover hover:scale-105 transition-transform duration-500">
                                            </div>
                                        @endif
                                    </div>
                                </div>
                    ...
                    @else
                        <div class="text-center py-16 bg-gray-50/50 rounded-3xl border border-dashed border-gray-200">
                            ...
                            <h4 class="text-lg font-bold text-gray-900 mb-1">Belum ada ulasan</h4>
                            <p class="text-gray-500 text-sm max-w-sm mx-auto">Jadilah yang pertama membagikan pengalaman Anda di destinasi ini!</p>
                        </div>
                    @endif
                </div>
