...
<div id="reviews" class="scroll-mt-32 section-block py-10" x-data="{ showReviewModal: false }">
    ...
    <div class="bg-white border border-gray-200 rounded-xl p-6 mb-6 relative overflow-hidden">
        <div class="flex items-center justify-between mb-4">
            <div>
                <h3 class="text-lg font-bold text-gray-900">Reviews summary</h3>
            ...
            <div class="flex items-center gap-2">
                ...
                <span class="text-xs font-bold text-gray-900">✨ Powered by AI</span>
            </div>
        </div>
        
        <div class="flex flex-col lg:flex-row gap-8">
            <div class="lg:w-2/3 text-sm text-gray-700 leading-relaxed space-y-4">
                <p>Visit Sukabumi offers an exciting and refreshing atmosphere that many travelers describe as breathtaking and deeply immersive, perfectly enhancing the overall holiday experience. While most reviewers find the local guides extremely attentive and welcoming, a few note occasional language barriers.</p>
                <p>The natural destinations receive rave reviews for their pristine condition and artistic natural formations, though some mention unpredictable weather affecting schedules.</p>
            ...
            <div class="lg:w-1/3 grid grid-cols-2 gap-3">
                <div class="border border-gray-200 rounded-lg p-3">
                    <div class="text-gray-900 mb-1"><svg class="w-4 h-4 inline-block mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>Vibe</div>
                    <div class="font-bold text-sm">Refreshing</div>
                </div>
                <div class="border border-gray-200 rounded-lg p-3">
                    <div class="text-gray-900 mb-1"><svg class="w-4 h-4 inline-block mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"/></svg>Service</div>
                    <div class="font-bold text-sm">Attentive</div>
                </div>
                <div class="border border-gray-200 rounded-lg p-3">
                    <div class="text-gray-900 mb-1"><svg class="w-4 h-4 inline-block mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>Wait time</div>
                    <div class="font-bold text-sm">Unpredictable</div>
                </div>
                <div class="border border-gray-200 rounded-lg p-3">
                    <div class="text-gray-900 mb-1"><svg class="w-4 h-4 inline-block mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"/></svg>Value</div>
                    <div class="font-bold text-sm">High-end</div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="mb-10">
        <div class="flex items-center justify-between mb-4">
            <h3 class="text-lg font-bold text-gray-900">Traveler photos ({{ $photos->count() }})</h3>
            <span class="text-sm font-semibold text-gray-900 underline cursor-pointer hover:text-black">Add photos</span>
        </div>
    ...
    <div>
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
            <h3 class="text-lg font-bold text-gray-900">All reviews ({{ number_format($totalReviews) }})</h3>
            @auth
                @if(!$userHasReviewed)
        ...
        <p class="text-xs text-gray-500 mb-6">Reviews are the subjective opinion of Visit Sukabumi members and not of Visit Sukabumi LLC. Visit Sukabumi performs checks on reviews as part of our industry-leading trust & safety standards.</p>
        ...
        <div class="flex flex-wrap items-center gap-3 mb-6">
            ...
            <div class="relative flex-1 min-w-[200px]">
                ...
                <input type="text" placeholder="Search reviews" class="w-full pl-9 pr-4 py-2 bg-white border border-gray-300 rounded-full text-sm focus:ring-[#002f20] focus:border-[#002f20] transition-colors">
            </div>
        </div>

        {{-- Popular Mentions (Dummy) --}}
        <div class="mb-8">
            <h4 class="text-sm font-bold text-gray-900 mb-3">Popular mentions</h4>
            <div class="flex flex-wrap gap-2">
                @foreach(['view', 'local guide', 'atmosphere', 'affordable', 'clean', 'must visit', 'culture', 'nature', 'hidden gem', 'family'] as $tag)
        ...
        @if($reviews->count() > 0)
            <div class="space-y-6">
                ...
                <div class="bg-white border border-gray-200 rounded-xl p-5 md:p-6 shadow-sm">
                    <div class="flex items-start justify-between mb-4">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <div class="text-xs text-gray-500">Sukabumi, Indonesia • {{ rand(1, 15) }} contributions</div>
                            </div>
                        </div>
                        <div class="flex items-center gap-3">
                    ...
                    <h4 class="font-bold text-gray-900 mb-1">Unforgettable experience!</h4>
                    ...
                    @endif
                    <div class="mt-4 text-xs text-gray-500">Written {{ $review->created_at->translatedFormat('F j, Y') }}</div>
                </div>
                @endforeach
            </div>
        @else
            <div class="text-center py-12 border border-gray-200 rounded-xl bg-gray-50">
                <h4 class="font-bold text-gray-900 mb-2">No reviews yet</h4>
    ...
    <div x-show="showReviewModal" 
         ...
         <div @click.away="showReviewModal = false" 
            ...
            <div class="flex items-center justify-between p-6 border-b border-gray-100">
                <h3 class="text-xl font-bold text-gray-900">Tell us how it was</h3>
            ...
            <div class="p-6 overflow-y-auto">
                <form action="{{ $storeRoute }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-8">
                        <label class="block text-sm font-bold text-gray-900 mb-3">How would you rate your experience?</label>
                    ...
                    <div class="mb-6">
                        <label class="block text-sm font-bold text-gray-900 mb-2">Write your review</label>
                        <textarea name="content" rows="5" class="w-full rounded-xl border border-gray-300 focus:border-[#002f20] focus:ring focus:ring-[#002f20]/20 text-gray-900 p-4 transition-all resize-none text-[15px]" placeholder="What was the highlight of your visit? Let others know what to expect." required></textarea>
                        <div class="text-right text-xs text-gray-500 mt-2">Minimum 100 characters</div>
                    </div>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                        <div>
                            <label class="block text-sm font-bold text-gray-900 mb-2">Who was with you? (Optional)</label>
                            <select name="visit_type" class="w-full rounded-xl border border-gray-300 focus:border-[#002f20] focus:ring focus:ring-[#002f20]/20 text-gray-900 p-3.5 transition-all appearance-none bg-[url('data:image/svg+xml;charset=US-ASCII,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20width%3D%22292.4%22%20height%3D%22292.4%22%3E%3Cpath%20fill%3D%22%23131313%22%20d%3D%22M287%2069.4a17.6%2017.6%200%200%200-13-5.4H18.4c-5%200-9.3%201.8-12.9%205.4A17.6%2017.6%200%200%200%200%2082.2c0%205%201.8%209.3%205.4%2012.9l128%20127.9c3.6%203.6%207.8%205.4%2012.8%205.4s9.2-1.8%2012.8-5.4L287%2095c3.5-3.5%205.4-7.8%205.4-12.8%200-5-1.9-9.2-5.5-12.8z%22%2F%3E%3C%2Fsvg%3E')] bg-[length:12px_12px] bg-no-repeat bg-[position:right_1rem_center]">
                                ...
                                <option value="Pasangan">Couples</option>
                                <option value="Teman">Friends</option>
                                <option value="Solo">Solo</option>
                                <option value="Bisnis">Business</option>
                            </select>
                        </div>
                        <div>
                            <label class="block text-sm font-bold text-gray-900 mb-2">Add a photo (Optional)</label>
                            <div class="relative w-full h-[52px]">
                                <input type="file" name="image" id="modal-review-image" accept="image/*" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10" onchange="document.getElementById('modal-file-name').textContent = this.files[0] ? this.files[0].name : 'Choose a file'"/>
                                <div class="w-full h-full rounded-xl border border-gray-300 bg-white flex items-center justify-center gap-2 p-3 text-gray-900 font-semibold text-sm transition-colors group-hover:border-[#002f20]">
                                    ...
                                    <span id="modal-file-name" class="truncate">Add a photo</span>
                                </div>
                            </div>
                        </div>
                    </div>
